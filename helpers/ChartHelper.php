...
class ChartHelper {
    ...
    public static function generarLineChart(array $data, array $labels, string $titulo = '', int $width = 600, int $height = 400): string {
        ...
        // Validar que haya al menos 2 puntos para líneas
        if($numPuntos < 2) {
            // Si hay 0 o 1 punto, dibujar mensaje
            $msg = $numPuntos === 0 ? 'Sin datos' : 'Datos insuficientes (minimo 2 puntos)';
            imagestring($img, 3, ($width - strlen($msg) * imagefontwidth(3)) / 2, $height / 2, $msg, $negro);
            ob_start();
            imagepng($img);
            $contenido = ob_get_clean();
    ...
    public static function generarPieChart(array $data, array $labels, string $titulo = '', int $width = 500, int $height = 400): string {
        ...
        foreach($data as $i => $valor) {
            ...
            imagefilledarc($img, $centroX, $centroY, $radio * 2, $radio * 2, 
                          $anguloInicio, $anguloInicio + $angulo, $color, IMG_ARC_PIE);
        ...
        foreach($data as $i => $valor) {
            ...
            // Cuadrado de color
            imagefilledrectangle($img, $leyendaX, $leyendaY + ($i * 30), 
                                $leyendaX + 15, $leyendaY + ($i * 30) + 15, $color);
