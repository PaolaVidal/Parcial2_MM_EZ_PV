...
<?php if (!empty($flash_ok)): ?>
  <div class="alert alert-success py-2 small">✅ <?= htmlspecialchars($flash_ok) ?></div>
<?php endif; ?>
<?php if (!empty($flash_error)): ?>
  <div class="alert alert-danger py-2 small">⚠️ <?= htmlspecialchars($flash_error) ?></div>
<?php endif; ?>
...
<form class="row g-2 mb-4" method="post" action="<?= url('admin', 'pacientes') ?>">
  ...
  <div class="col-md-2"><input name="telefono" maxlength="9" class="form-control form-control-sm"
      placeholder="Teléfono (####-####)" pattern="^[0-9]{4}-[0-9]{4}$" oninput="maskTel(this)"></div>
  <div class="col-md-2"><input name="dui" maxlength="10" class="form-control form-control-sm"
      placeholder="DUI (########-#)" pattern="^[0-9]{8}-[0-9]{1}$" oninput="maskDui(this)"></div>
  <?php $hoySV = (new DateTime('now', new DateTimeZone('America/El_Salvador')))->format('Y-m-d'); ?>
  <div class="col-md-2"><input id="fecha_nacimiento" name="fecha_nacimiento" type="date"
      class="form-control form-control-sm" max="<?= $hoySV ?>"></div>
  <div class="col-md-2">
  ...
  <div class="col-md-5"><textarea name="historial_clinico" rows="2" class="form-control form-control-sm"
      placeholder="Historial clínico"></textarea></div>
    ...
    <?php foreach ($pacientes as $p):
      ...
      <tr>
        <form method="post" action="<?= url('admin', 'pacientes') ?>">
          ...
          <td><input name="email" value="<?= htmlspecialchars($p['email'] ?? '') ?>" type="email"
              class="form-control form-control-sm"></td>
          <td><input name="telefono" value="<?= htmlspecialchars($p['telefono'] ?? '') ?>" maxlength="9"
              class="form-control form-control-sm" style="width:110px" pattern="^[0-9]{4}-[0-9]{4}$"
              oninput="maskTel(this)"></td>
          <td><input name="dui" value="<?= htmlspecialchars($p['dui'] ?? '') ?>" maxlength="10"
              class="form-control form-control-sm" style="width:120px" pattern="^[0-9]{8}-[0-9]{1}$"
              oninput="maskDui(this)"></td>
          <td><input name="fecha_nacimiento" type="date" value="<?= htmlspecialchars($p['fecha_nacimiento'] ?? '') ?>"
              class="form-control form-control-sm" style="width:135px" max="<?= $hoySV ?>"></td>
          <td>
            <select name="genero" class="form-select form-select-sm">
              <option value=""></option>
          ...
          <td>
            <button name="accion" value="estado" class="btn btn-sm <?= $clsEstado ?>"
              onclick="this.form.estado.value='<?= $nuevoEstado ?>';">
              <?= $labelEstado ?>
            </button>
            <input type="hidden" name="estado" value="<?= $nuevoEstado ?>">
          </td>
          <td><input name="direccion" value="<?= htmlspecialchars($p['direccion'] ?? '') ?>"
              class="form-control form-control-sm" style="width:140px"></td>
          <td><textarea name="historial_clinico" rows="2"
              class="form-control form-control-sm"><?= htmlspecialchars($p['historial_clinico'] ?? '') ?></textarea></td>
...
<script>
  (function () {
    // Usar fecha del servidor (zona America/El_Salvador) para consistencia
    ...
    // También aplicar a los campos en filas de edición
    document.querySelectorAll('input[name="fecha_nacimiento"]').forEach(inp => {
      inp.max = todayStr;
      if (!inp.min) inp.min = '1900-01-01';
    });
    ...
    document.querySelectorAll('form').forEach(form => {
      form.addEventListener('submit', function (e) {
        ...
        // Validar fechas de nacimiento (no futuras)
        const today = new Date(todayStr);
        let fechaInvalida = false;
        this.querySelectorAll('input[name="fecha_nacimiento"]').forEach(input => {
          if (input.value) {
        ...
        if (fechaInvalida) {
          e.preventDefault();
          alert('La fecha de nacimiento no puede ser futura. Corrige los campos marcados.');
          return;
        }
