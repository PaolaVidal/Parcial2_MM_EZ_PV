...
<?php if (isset($_GET['ok'])): ?>
  <div class="alert alert-success py-1">Operación realizada.</div><?php endif; ?>
<?php if (isset($_GET['err'])): ?>
  <div class="alert alert-danger py-1">Error: <?= htmlspecialchars($_GET['err']) ?></div><?php endif; ?>
<form class="row g-2 mb-3" method="get" action="<?= RUTA ?>index.php">
  <input type="hidden" name="url" value="admin/horarios">
  <div class="col-md-4">
    <select name="ps" class="form-select form-select-sm" onchange="this.form.submit()">
      ...
      <?php foreach ($psicologos as $p): ?>
        <option value="<?= (int) $p['id'] ?>" <?= $idSel == $p['id'] ? 'selected' : '' ?>>
          <?= htmlspecialchars($p['nombre'] ?? ('Psicólogo #' . $p['id'])) ?>
        </option>
      <?php endforeach; ?>
    </select>
  </div>
  ...
  <table class="table table-sm table-striped">
    ...
    <tbody>
      <?php if (empty($horarios)): ?>
        <tr>
          <td colspan="4" class="text-muted">Sin horarios definidos para este psicólogo.</td>
        </tr>
      <?php else:
        foreach ($horarios as $h): ?>
          <tr>
            ...
            <td>
              <form method="post" action="<?= url('admin', 'horarios') ?>" onsubmit="return confirm('Eliminar horario?');"
                class="d-inline">
