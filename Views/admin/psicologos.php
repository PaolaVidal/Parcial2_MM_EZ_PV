...
<?php if(!empty($error)): ?><div class="alert alert-danger py-1 mb-2"><?= htmlspecialchars($error) ?></div><?php endif; ?>
...
<?php if(empty($psicologos)): ?><div class="alert alert-info">Sin psicólogos.</div><?php endif; ?>

<h2 class="h6 mt-4">Más solicitados</h2>
<table class="table table-sm">
  <thead><tr><th>Psicólogo</th><th>Citas</th></tr></thead>
  <tbody>
  <?php foreach($masSolic as $m): ?>
    <tr><td><?= htmlspecialchars($m['nombre']??($m['id_psicologo']??'')) ?></td><td><?= $m['total'] ?? $m['citas'] ?? '' ?></td></tr>
  <?php endforeach; ?>
  </tbody>
</table>