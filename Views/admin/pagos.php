  ...
  <div class="col-md-6">
    <div class="card card-body">
      ...
      <table class="table table-sm mb-0">
        <thead><tr><th>ID</th><th>Cita</th><th>Monto</th><th>Fecha</th></tr></thead>
        <tbody>
        <?php foreach($pendientes as $p): ?>
          <tr><td><?= $p['id'] ?></td><td><?= $p['id_cita'] ?></td><td>$<?= number_format($p['monto_total'],2) ?></td><td><?= $p['fecha'] ?></td></tr>
        <?php endforeach; ?>
        <?php if(empty($pendientes)): ?><tr><td colspan="4" class="text-center text-muted">Sin pendientes</td></tr><?php endif; ?>
        </tbody>
      </table>
    </div>
  </div>
