...
class TicketController
{
    ...
    public function qr($id = 0): void
    {
        ...
        if (!is_file($rutaFs)) {
            http_response_code(404);
            echo 'QR no disponible';
            return;
        }
        $download = isset($_GET['dl']);
        header('Content-Type: image/png');
        if ($download) {
            header('Content-Disposition: attachment; filename="ticket_' . $id . '.png"');
        }
    ...
    public function pdf($id = 0): void
    {
        ...
        try {
            ...
            // Log the exception and return a generic 500 without echoing to avoid corrupting binary/pdf output
            error_log('TicketController::pdf error generating PDF: ' . $e->getMessage());
