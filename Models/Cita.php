...
class Cita extends BaseModel
{
    ...
    public function confirmarPorQR(string $token, int $idPsicologo): array
    {
        ...
        if (!$cita)
            return ['ok' => false, 'msg' => 'QR no encontrado'];
        if ((int) $cita['id_psicologo'] !== $idPsicologo)
            return ['ok' => false, 'msg' => 'No pertenece a este psicólogo'];
        if ($cita['estado_cita'] === 'realizada')
            return ['ok' => true, 'msg' => 'Ya estaba marcada realizada', 'cita' => $cita];
        if ($cita['estado_cita'] !== 'pendiente')
            return ['ok' => false, 'msg' => 'Estado no permite confirmación'];
    ...
    {
        // La columna 'especialidad' puede no existir en la tabla Psicologo; normalmente se usa 'id_especialidad'.
        // Intentamos acceder al nombre de la especialidad mediante LEFT JOIN a Especialidad si está presente.
        $sql = "SELECT c.*, 
