...
class Psicologo extends BaseModel
{
    ...
    /**
     * Listar psicólogos activos con filtros opcionales
     * @param string $nombre Texto a buscar en el nombre del usuario
     * @param int|null $idEspecialidad Filtrar por especialidad específica
     * @return array
     */
    public function listarActivosFiltrado(string $nombre = '', ?int $idEspecialidad = null): array
    {
        $where = ["u.estado='activo'", "u.rol='psicologo'"];
        $params = [];
        if ($nombre !== '') {
        ...
        if ($idEspecialidad) {
            $where[] = 'p.id_especialidad = :esp';
            $params[':esp'] = $idEspecialidad;
        }
        $sql = "SELECT p.*, u.nombre, u.email, u.estado, e.nombre AS nombre_especialidad
        FROM {$this->tPsicologo} p
        JOIN {$this->tUsuario} u ON u.id = p.id_usuario
        LEFT JOIN Especialidad e ON e.id = p.id_especialidad
        WHERE " . implode(' AND ', $where) . " ORDER BY u.nombre";
        $st = $this->db->prepare($sql);
        $st->execute($params);
